...
Stay inside safe areas. Top-level browsing uses a two-item `TabView` for Quilt and Events, with a `NavigationStack` in each tab. Quilt selection and filtering use dismissible sheets with explicit Done/Cancel actions. Detail screens use inline navigation titles and preserve the edge-swipe back gesture.

The Quilt tab presents a native segmented Quilt/Map/List control. Quilt uses `QuiltCanvas`, a UIKit `UIScrollView` with zoom and pan; its minimum gesture zoom is 0.3 and its initial fit is clamped to at least 0.65. Filtering matches selected tags with OR semantics and intersects them with case and diacritic insensitive name/description search. Matching patches repack from their original sizes, while clearing filters restores the baseline arrangement. Map uses actual coordinates and the same narrowed collection; List uses quilt placement order, Name, or Newest.
...
The canvas fills the available iPhone or iPad viewport. Profiles use native sheets on both device classes; a persistent side-by-side profile is not implemented.
...
**The Live Surface Rule.** Opening a profile keeps the quilt's filter and viewport alive behind it. Filtering never dismisses the profile; changing quilts does.
...
- **Style:** a labeled native control opens a list sheet of interests with checkmarks and a Done action.
- **State:** the active count is visible in the filter label; Clear removes search and interests together. Map, Quilt, and List read the same state.
...
The patch's own profile opens in a native sheet with medium and large detents, adapting its presentation on iPad. It uses a navigation stack for Upcoming events, native ShareLink, Apple Maps directions, and a website link. Done dismisses the sheet. The underlying discovery state remains intact.

### Navigation

Use SF Symbols, `TabView`, `NavigationStack`, grouped `List`, `Map`, native sheets, and system materials. Quilt choice is always explicit; Lancaster is a directory entry, never an implicit launch selection.
...
- **Don't** silently select a quilt or invent authenticated actions; joining, following, and governance remain website links in this browsing pass.
